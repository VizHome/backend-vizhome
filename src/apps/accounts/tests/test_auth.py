...
class TestRegister:
    ...
    def test_register_rejects_password_mismatch(self, api_client: APIClient):
        response = api_client.post(self.url, {
            'email': 'x@x.fr', 'password': 'Test1234!', 'password_confirm': 'OTHER1234!',
        }, format='json')
...
class TestLogin:
    ...
    def test_login_creates_session(self, api_client: APIClient, user: User):
        api_client.post(self.url, {
            'email': user.email, 'password': 'Test1234!',
        }, format='json', HTTP_USER_AGENT='Mozilla/5.0 (Windows) Chrome/120')
...
@pytest.mark.django_db
class TestLogout:
    def test_logout_blacklists_refresh_and_revokes_session(
...
class TestPasswordReset:
    ...
    def test_forgot_then_reset_password(self, api_client: APIClient, user: User, mailoutbox):
        ...
        # 3. Reset
        r2 = api_client.post('/api/v1/auth/reset-password', {
            'uid': uid, 'token': token,
