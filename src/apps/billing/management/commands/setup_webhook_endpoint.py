...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        endpoint = WebhookEndpoint.objects.filter(
            djstripe_owner_account__isnull=True
        ).first()
        if endpoint is None:
        ...
        self.stdout.write(
            self.style.SUCCESS(f"{prefix} : WebhookEndpoint {endpoint.djstripe_uuid}")
        )
        self.stdout.write("")
        self.stdout.write(
            self.style.WARNING("Utilise cette URL dans `stripe listen` :")
        )
        self.stdout.write(
            self.style.WARNING(f"  stripe listen --forward-to {full_url}")
        )
