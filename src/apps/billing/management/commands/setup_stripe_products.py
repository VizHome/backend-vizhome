...
class Command(BaseCommand):
    ...
    def handle(self, *args, dry_run: bool = False, **opts) -> None:
        ...
        if not billable:
            self.stdout.write(
                self.style.WARNING("Aucun plan billable dans PLAN_CONFIG.")
            )
            return
        ...
        for plan_name, config in billable.items():
            ...
            if dry_run:
                self.stdout.write(
                    self.style.WARNING("  [dry-run] Skip Stripe API calls")
                )
                continue

            # 1. Find or create Product (par metadata.plan)
            existing = stripe.Product.search(
                query=f'metadata["plan"]:"{plan_name}"', limit=1
            )
            if existing.data:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"  + Product créé : {product.id}")
                )
            ...
            if existing_prices.data:
                ...
                if price.unit_amount != config["price_eur"]:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"  + Nouvelle Price : {new_price.id}")
                    )
            else:
