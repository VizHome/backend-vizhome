...
def user_normal(db) -> User:
    return User.objects.create_user(
        email='normal@example.com', password='Secure!1234',
    )
...
def user_staff(db) -> User:
    return User.objects.create_user(
        email='admin@example.com', password='Secure!1234', is_staff=True,
    )
...
class TestOverviewShape:
    def test_response_has_all_top_level_sections(self, client_staff):
        ...
        expected_keys = {
            'generated_at',
            'users', 'sessions', 'renders', 'projects',
            'storage', 'billing', 'forum', 'system',
        }
    ...
    def test_system_section_reports_integrations(self, client_staff):
        ...
        s = r.data['system']
        for key in (
            'gemini_configured', 'stripe_configured', 'google_oauth_configured',
            'github_oauth_configured', 'minio_configured', 'sentry_configured',
        ):
            assert key in s
...
class TestMetricsComputation:
    ...
    def test_users_by_plan_distribution(self, client_staff):
        ...
        for i in range(3):
            User.objects.create_user(
                email=f'free{i}@example.com', password='Pass!1234', plan='free',
            )
        for i in range(2):
            User.objects.create_user(
                email=f'pro{i}@example.com', password='Pass!1234', plan='pro',
            )
        r = client_staff.get(API)
    ...
    def test_billing_mrr_from_paying_users(self, client_staff):
        ...
        for i in range(2):
            User.objects.create_user(
                email=f'mrr-pro-{i}@example.com', password='Pass!1234', plan='pro',
            )
        r = client_staff.get(API)
