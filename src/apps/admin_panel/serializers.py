...
class AdminUserSerializer(serializers.ModelSerializer):
    ...
    storage_used_bytes = serializers.IntegerField(
        source='stats.storage_used_bytes', read_only=True, default=0,
    )
    renders_this_month = serializers.IntegerField(
        source='stats.renders_this_month', read_only=True, default=0,
    )
    total_projects = serializers.IntegerField(
        source='stats.total_projects', read_only=True, default=0,
    )
...
class AdminRenderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            ...
            'source', 'output_type', 'status', 'provider',
            'prompt', 'title', 'error_message',
