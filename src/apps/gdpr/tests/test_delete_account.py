...
class TestRequestDeleteAccount:
    ...
    def test_creates_request_and_deactivates_user(
        ...
        r = auth_client.post(
            self.URL, {'confirm': 'DELETE', 'notes': 'au revoir'}, format='json'
        )
    ...
    def test_conflict_if_already_scheduled(
        self, auth_client: APIClient, user: User
    ):
    ...
    def test_replaces_old_cancelled_request(
        self, auth_client: APIClient, user: User
    ):
        DeletionRequest.objects.create(
            user=user, cancelled_at=timezone.now() - timedelta(days=1)
        )
        r = auth_client.post(self.URL, {'confirm': 'DELETE'}, format='json')
        assert r.status_code == 202
...
class TestCancelDeleteAccount:
    ...
    def test_cancels_pending_request_and_reactivates_user(
        self, auth_client: APIClient, user: User
    ):
    ...
    def test_410_when_already_completed(
        self, auth_client: APIClient, user: User
    ):
    ...
    def test_idempotent_when_already_cancelled(
        self, auth_client: APIClient, user: User
    ):
        DeletionRequest.objects.create(
            user=user, cancelled_at=timezone.now() - timedelta(hours=1)
        )
        r = auth_client.post(self.URL)
        assert r.status_code == 200

    def test_does_not_cancel_other_user_request(
