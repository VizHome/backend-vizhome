...
@pytest.mark.django_db
def test_list_includes_messages_count_annotation(
...
def test_get_other_user_ticket_returns_404(
    other_client: APIClient, ticket: SupportTicket
):
...
def test_staff_can_patch_status(staff_client: APIClient, ticket: SupportTicket):
    res = staff_client.patch(
        f"/api/v1/support/tickets/{ticket.pk}",
        {
            "status": "resolved",
        },
        format="json",
...
@pytest.mark.django_db
def test_message_author_exposes_pseudo(
    auth_client: APIClient, user: User, ticket: SupportTicket
):
