...
class TestEditWindowTopic:
    ...
    def test_author_cannot_edit_after_window(self, auth_client, topic):
        ...
        Topic.objects.filter(pk=topic.pk).update(created_at=timezone.now() - timedelta(minutes=16))
        r = auth_client.patch(
            f'{API}/topics/{topic.id}',
    ...
    def test_staff_can_edit_anytime(self, staff_client, topic):
        ...
        Topic.objects.filter(pk=topic.pk).update(created_at=timezone.now() - timedelta(days=30))
...
class TestEditWindowReply:
    ...
    def test_author_cannot_edit_reply_after_window(self, auth_client, user, topic):
        reply = Reply.objects.create(
            topic=topic,
            author=user,
            content='Vieille réponse.',
        )
        # Hors fenêtre
        Reply.objects.filter(pk=reply.pk).update(created_at=timezone.now() - timedelta(minutes=20))
        r = auth_client.patch(
            f'{API}/replies/{reply.id}',
            {'content': 'Trop tard.'},
            format='json',
    ...
    def test_author_cannot_delete_reply_after_window(self, auth_client, user, topic):
        reply = Reply.objects.create(
            topic=topic,
            author=user,
            content='Trop vieille.',
        )
        Reply.objects.filter(pk=reply.pk).update(created_at=timezone.now() - timedelta(hours=1))
        r = auth_client.delete(f'{API}/replies/{reply.id}')
        assert r.status_code == 403
